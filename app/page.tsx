...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      {/* --- SECTION 3: EDUKASI (FETCHED FROM API) --- */}
      <section className="h-screen w-full snap-start flex flex-col justify-center bg-slate-950 relative shrink-0">
        <div className="px-8 mb-6 text-center">
           <h2 className="text-3xl md:text-4xl font-bold text-white mb-2">Kenali Musuhmu</h2>
           <p className="text-slate-400">Geser ke samping untuk melihat berbagai jenis serangan</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="w-full overflow-x-auto snap-x snap-mandatory flex gap-6 px-8 pb-12 pt-4 no-scrollbar items-center">
            
            {phishingCategories.map((item, index) => (
              <div 
                key={index}
                className="snap-center shrink-0 w-[350px] md:w-[400px] bg-slate-900/50 border border-slate-800 p-8 rounded-3xl hover:border-emerald-500/30 transition-all duration-300 shadow-xl flex flex-col h-[450px]"
              >
                <div className="flex items-center gap-4 mb-6">
                  <div className="p-3 bg-slate-950 rounded-2xl border border-slate-800">
                    {/* Panggil fungsi helper icon */}
                ...
                {mitigationData[activeTab]?.map((item: any, index: number) => (
                  ...
                  >
                    ...
                    <div className={`px-5 pb-5 transition-all duration-300 ${
                      openAccordion === index ? 'block opacity-100' : 'hidden opacity-0 h-0'
                    }`}>
      ...
      <section className="h-screen w-full snap-start flex flex-col items-center justify-center relative p-4 bg-gradient-to-b from-slate-950 to-emerald-950/30 shrink-0">
        ...
        <footer className="absolute bottom-8 text-slate-600 text-sm">© 2025 PhishGuard AI Project</footer>
      </section>
